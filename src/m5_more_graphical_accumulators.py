...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(n):
        circle = rg.Circle(rg.Point(rectangle.get_center().x, rectangle.corner_1.y-k*(radius*2)-radius), radius)
        circle.attach_to(window)
        window.render(.1)
    for k in range(m):
        circle = rg.Circle(rg.Point(rectangle.corner_1.x-k*(radius1*2)-radius1, rectangle.get_center().y), radius1)
        circle.attach_to(window)
        window.render(.1)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    p1 = rg.Point(rectangle1.get_center().x, rectangle1.get_center().y)
    p2 = rg.Point(rectangle2.get_center().x, rectangle2.get_center().y)
    window.render(.1)
    for k in range(n):
        line = rg.Line(rg.Point(p1.x-(k*(rectangle1.get_width()/2)), p1.y+(k*(rectangle1.get_height()/2))), rg.Point(p2.x-(k*(rectangle1.get_width()/2)), p2.y+(k*(rectangle1.get_height()/2))))
        line.thickness = 5
        if k % 2 == 0:
